...
```
**This step only means anything because of the manual offset**, `filter_safe_variants` takes positions at face value, and `suggest_variant`'s
...
1. **The K_CK documentation typo: `~3e-12` → `3.32e-5`.** My source PDFs
   ...
   (`exp(-(10.905-4.8)/0.592) ≈ 3.32e-5`) confirmed `3.32e-5` is correct, about 3300x *weaker*, consistent with the binder's documented "doesn't
