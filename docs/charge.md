...
**Why it exists:** LOCKR binders are built from designed helical bundles, if a substitution (like the D/E→A neutralizing fix) accidentally introduced a
...
**Gotchas:** This is a *statistical propensity*, not a structure prediction, it says nothing about whether the sequence will actually fold as designed in
...
**What it does:** Finds every internal Proline or Glycine in the sequence, the two residues most likely to physically kink a helix.
...
**The math/logic:** `[i for i, aa in enumerate(seq, 1) if aa in "PG" and 1 < i < len(seq)]`, the `1 < i < len(seq)` condition is what excludes the first and last
...
(`BinderSequence.residues()`, `liability.py`'s mutation positions, etc.), but unlike `liability.py`'s mutation positions, these are *always*
