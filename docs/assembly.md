...
relative to `full_sequence` (since `start`/`end` are themselves absolute), this is different from `liability.py`'s mutation positions, which are
...
intersects it against `[protected_region.start, protected_region.end]`, `lo = max(seg_start, region.start)`, `hi = min(seg_end, region.end)`; if
...
```
17 + 1 (G linker) + 17 = 35, exactly filling the window with zero slack, matching Script 5's documented "v2.2 length: 359 aa OK."
...
real v1.0 case (length, protected-region, overlap, fit, spacer, binder1, matching Script 6's six lines exactly), because v1.0 has no
...
**The math/logic:** `int("".join(ch for ch in mutation if ch.isdigit()))`, relies on residue codes always being letters and positions always being
...
*unoffset*, but after applying a real offset, lands at absolute position 41, outside the region, and gets accepted. That test exists specifically to
