...
**Why it exists: this is the actual scientific claim of the whole module**, that charged residues on a LOCKR binder don't just sit there neutrally, they
...
record a mutation string in `"{old}{position}{new}"` format (e.g. `"D4A"`, note this string format is exactly what `assembly.py`'s `_mutation_position`
